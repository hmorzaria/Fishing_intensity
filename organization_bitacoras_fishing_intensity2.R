...
paththisLocation = paste(datapath,name.sp.folder,sep="/")
  ...
  #make loc list for this folder
  loc.list = matrix(0,0,1) %>% 
    data.frame %>% setnames("LOC")
  
  for(eachname in 1:length(shape.species))
  {
    setwd(paththisLocation)
    this.name = shape.species[eachname] %>% as.character()
    shape.file.name = unlist(strsplit(this.name,"-"))[1]
     sitiocode = unlist(strsplit(shape.file.name,""))[1:3] 
     sitiocode = paste(sitiocode[1],sitiocode[2],sitiocode[3],sep="")
     sitiocode = data.frame(sitiocode)
     loc.list = rbind(loc.list,sitiocode)
    }
  
  unique.loc = unique(loc.list[1]) %>% .$sitiocode %>% as.character
  
  for(eachloc in 1:length(unique.loc))

      {
    setwd(paththisLocation)
    
    this.loc = unique.loc[eachloc]
    
    shape.sp=list.files(getwd(), pattern=glob2rx(paste(this.loc,"*.shp",sep="")), full.names=FALSE)
  ...
  writeRaster(sp.r, filename=paste(this.loc,name.sp.folder,"bitac_sp",sep="_"), format="GTiff", overwrite=TRUE)  
...
# Now sum all rasters
  ...
  setwd(pathToSaveShapes)
   ...
   raster.files = list.files(getwd(),  pattern="Indx.tif$", full.names=FALSE)
    ...
    raster.files = list.files(getwd(),  pattern="IE.tif$", full.names=FALSE)
    raster.species.comm = stack(raster.files)
    raster.species.comm[is.na(raster.species.comm)] <- 0
