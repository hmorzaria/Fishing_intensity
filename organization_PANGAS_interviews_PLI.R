...
pathToSaveShapes = "E:/Archivos/SIG/Proyectos/ArcGis/Datos ordenados/Shapes y layers/Spatial management/Pangas_interviews_9km_corridor/By_species_PLI"
...
corridor.com <- all.rg[all.rg$sitio_code == "PLI",]
...
for (eachspecies in 1:length(sp.levels))
{
  ...
  if(TRUE %in% test.sp==TRUE) {
    ...
    setwd(pathToSaveShapes)
    writeRaster(out.r, filename=paste("all_interviews",this.species,sep="_"), format="GTiff", overwrite=TRUE)  
