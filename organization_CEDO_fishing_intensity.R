...
print(this.sp)
    ...
    for(eachline in 1:nrow(sp.data))
      
    {
      sp.data.line = sp.data[eachline,]
    
    coordinates(sp.data.line) = c("LON","LAT")
    
    crs.wgs <- CRS("+proj=longlat +ellps=WGS84 +datum=WGS84")  # geographical, datum WGS84
    proj4string(sp.data.line) <- crs.wgs 
    
    sp.data.geo = spTransform(sp.data.line, crs.geo)
